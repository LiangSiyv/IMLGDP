...
class ResNet3dSlowOnly2sFLC3(nn.Module):
    ...
    def forward(self, x, x_local):
        ...
        x_local = x_local.view(x_local.size(0), -1)    # shape=[1,2048,1,1,1] -> shape=[1,8,256,1,1]


        # [N x C]
        x_full = x_full.view(x_full.size(0), -1)    # shape=[1,2048,1,1,1] -> shape=[1,2048,1,1]


        out = torch.cat((x_full, x_local),dim=1)

        return out


if mmdet_imported:
    MMDET_BACKBONES.register_module()(ResNet3dSlowOnly2sFLC3)
