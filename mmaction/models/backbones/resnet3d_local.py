...
from .resnet3d import ResNet3dLayer
...
class BasicBlock3dLocal(nn.Module):
...
class Bottleneck3dLocal(nn.Module):
...
class ResNet3dLocal(nn.Module):
    ...
    arch_settings = {
        18: (BasicBlock3dLocal, (2, 2, 2, 2)),
        34: (BasicBlock3dLocal, (3, 4, 6, 3)),
        50: (Bottleneck3dLocal, (3, 4, 6, 3)),
        101: (Bottleneck3dLocal, (3, 4, 23, 3)),
        152: (Bottleneck3dLocal, (3, 8, 36, 3))
    }
    ...
    def _init_weights(self, pretrained=None):
        ...
        if isinstance(self.pretrained, str):
            ...
            if self.pretrained2d:
                ...
                checkpoint_new = {}
                unmatch_keys = []
                for (name1, data1), (name2, data2) in zip(checkpoint.items(), model_dict.items()):
                    # print(name1,data1.shape,name2,data2.shape)
                    if data1.shape==data2.shape:
                        checkpoint_new[name2] = data1
                    elif data1.shape.numel()==data2.shape.numel():
                        checkpoint_new[name2] = data1.view(data2.shape)
                    elif data1.shape[-2:]==data2.shape[-2:] and data1.shape[:2]==data2.shape[:2]:
                        if data2.shape[2] < data1.shape[2]:
                            data1 = data1[:, :, :data2.shape[2], :, :]
                            checkpoint_new[name2] = data1.view(data2.shape)
                        else:
                            unmatch_keys.append(name2)
                    elif data1.shape[-2:]==data2.shape[-2:] and data1.shape[:1]==data2.shape[:1]:
                        rate = data2.shape[1] / data1.shape[1]
                        data1 = data1[:, :int(data1.shape[1] * rate), :, :, :]
                        if data1.shape[2] < data2.shape[2]:
                            repeat_shape = [1 for _ in range(len(data1.shape))]
                            repeat_shape[2] = int(data2.shape.numel()/data1.shape.numel())
                            checkpoint_new[name2] = data1.repeat(repeat_shape).view(data2.shape)
                        else:
                            data1 = data1[:, :,:data2.shape[2], :, :]
                            checkpoint_new[name2] = data1.view(data2.shape)

                    elif data2.shape.numel()/data1.shape.numel() in [2.0, 4.0]:
                        repeat_shape = [1 for _ in range(len(data1.shape))]
                        repeat_shape[0] = int(data2.shape.numel()/data1.shape.numel())
                        checkpoint_new[name2] = data1.repeat(repeat_shape).view(data2.shape)
                    elif data2.shape.numel()/data1.shape.numel() in [0.25, 0.125]:
                        rate = data2.shape.numel() / data1.shape.numel()
                        data1 = data1[:, :int(data1.shape[1] * rate), :, :, :]
                        checkpoint_new[name2] = data1.view(data2.shape)
                    else:
                        unmatch_keys.append(name2)
                ...
                if unmatch_keys==[]:
                    ...
                    logger.info(f'unmatchkeys:{unmatch_keys}')
            ...
            if self.zero_init_residual:
                for m in self.modules():
                    if isinstance(m, Bottleneck3dLocal):
                        constant_init(m.conv3.bn, 0)
                    elif isinstance(m, BasicBlock3dLocal):
...
if mmdet_imported:
    ...
    MMDET_BACKBONES.register_module()(ResNet3dLocal)
